...
def read_input(num_particles, filename):
    ...
    y0 = np.zeros(79+60*num_particles+num_particles)
    ...
    partMC_kappas = np.array((temp[:]*1))
    
    for i in range(0, len(partMC_kappas)):
        if partMC_kappas[i] == 0.:
            partMC_kappas[i] = 1.0E-5
    ...
    for i in range(0, num_particles):
        ...
        name = 'Ddry, ' + str(i+1)
        y0[indexing.getIndex(name, num_particles)] = np.log(Ddry)
        dry_Ds.append(Ddry)
        ...
        name = 'Dp, ' + str(i+1)
        y0[indexing.getIndex(name, num_particles)] = np.log(Dp[0])
        Rp = 0.5*Dp[0]
        Rdry = 0.5*Ddry
