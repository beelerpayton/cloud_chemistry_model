...
def particle_phase(y, dydt, bins, Kelvin_effect = True, constant_pH = False, aq_oxidation = True):
    ...
    for i in range(0, bins):
        ...
        if V_water > 0:
            ...
            dydt[index] = molec_masses['HO2']*V_water*(dHO2)        
            ...
            Dg = (1/100**2)*1.9*np.power(molec_masses[name], (-2/3)) # est. gas-phase diffusion coefficient, m2/sec  (Schnoor 1996 via Lim)
            kmt = np.power((radius**2/(3*Dg))+((4.0*radius)/(3.0*w*alphas[name])), -1.0) # mass uptake, 1/s
            dIEPOX_gas = molec_masses[name]*V_water*(((kmt*Pi)/(R*T)) - ((kmt*Ci)/(H*R*T)))
            air_changes[index] = N*V_water*((R*T)/P)*(((kmt*Pi)/(R*T)) - ((kmt*Ci)/(H*R*T)))
            ...
            k_HSO4 = 7.3E-4 # 1/(M*s)
            k_NH4 = 1.7E-5 # 1/(M*s)
            c_SO4 = y[79+31*bins+i]/(1000*molec_masses['SO4--']*V_water) # mol/L
            c_HSO4 = y[79+30*bins+i]/(1000*molec_masses['HSO4-']*V_water) # mol/L
            c_NH4 = y[79+35*bins+i]/(1000*molec_masses['NH4+']*V_water) # mol/L
            k1 = ((c_H2O/55.1)*k_Hplus*Hplus_activity)\
                + (k_SO4*c_SO4*Hplus_activity)\
                + (k_HSO4*c_HSO4)\
                + (k_NH4*c_NH4) # 1/s
            dIEPOX_reactions = Ci*k1
            dydt[index] = molec_masses[name]*V_water*(dIEPOX_gas - dIEPOX_reactions)
            
            # tetrol
            name = 'tetrol'
            index = 79+58*bins+i
            dtetrol = (1-BETA)*dIEPOX_reactions
            dydt[index] = molec_masses[name]*V_water*(dtetrol)
            
            # IEPOXOS
            name = 'IEPOXOS'
            index = 79+59*bins+i
            dIEPOXOS = BETA*dIEPOX_reactions # mol/m^3*s
            dydt[index] = molec_masses[name]*V_water*(dIEPOXOS) #kg/s
            dSO4_air = dydt[79+31*bins+i]/(molec_masses['SO4--']*V_water) # mol/m^3*s
            dSO4_reactions = -1.0*dIEPOXOS # mol/m^3*s
            dydt[79+31*bins+i] = molec_masses['SO4--']*V_water*(dSO4_air + dSO4_reactions) #kg/s
