...
def parcel_properties(y, dydt, V, bins, thermal_accom, condens_coeff, constant_pressure=False):
    ...
    for i in range(0, bins):
        dry_radii.append(y[79+0*bins+i]/2.0)
        radii.append(y[79+1*bins+i]/2.0)
    ...
    if constant_pressure==True or Ntot==0:
        ...
        for N, radius, dry_radius, kappa in zip(Ns, radii, dry_radii, kappas):
            ...
            if N > 0:
                ...
                G = 1.0 / (G_a + G_b)
                kappa = kappa
                a_w = 1.0 / (1.0+kappa*(np.power(dry_radius/radius,3)))
            ...
            if radius <= dry_radius and dr_dt < 0:
                dr_dt = 0
...
def particle_changes(y, dydt, bins, co_condens):
    ...
    for i in range(0, bins):
        ...
        dydt[79+0*bins+i] = 2.0*(dVdry_dt/(4.0*np.pi*dry_radius**2))
        dydt[79+1*bins+i] = 2.0*(dVtot_dt/(4.0*np.pi*radius**2))
        ...
        if co_condens == True:
            ...
            dydt[79+2*bins+i] = (1/(Vtot**2))*summation
